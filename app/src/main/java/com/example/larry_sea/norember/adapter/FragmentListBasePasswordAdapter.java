 ...
 * <p/>
 * basepassword list 的适配器
 */
public class FragmentListBasePasswordAdapter extends RecyclerView.Adapter<SingleTitleViewHolder> implements StickyRecyclerHeadersAdapter<RecyclerView.ViewHolder> {
    //数据集
    RealmResults<BasePassword> basePasswordRealmResults;
    Context mcontext;
    RecyclerviewClickInterface mrecyclerViewInterface;

    public FragmentListBasePasswordAdapter(Context context, RealmResults<BasePassword> basePasswordsResults) {
        mcontext = context;
    ...
    public void onBindViewHolder(final SingleTitleViewHolder holder, int position) {
        ...
        Picasso.with(mcontext).load(R.mipmap.ic_type_password_medium).into(holder.iconIv);
        holder.titleTv.setText(basePasswordRealmResults.get(position).getTitle());
    ...
    public void onBindHeaderViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (basePasswordRealmResults.get(position).getTitle() != null) {
            textview.setText(basePasswordRealmResults.get(position).getTitle().substring(0, 1));
        } else {
